...
USER techflow
EXPOSE 8080
CMD ["sh", "-c", "if [ -n \"$DATABASE_URL_RAW\" ]; then db_target=\"${DATABASE_URL_RAW#*://}\"; export DATABASE_URL=\"jdbc:postgresql://${db_target#*@}\"; fi; if [ -n \"$PORT\" ]; then export SERVER_PORT=\"$PORT\"; fi; export PYTHONPATH=/app; (ulimit -v 250000 2>/dev/null; while true; do python3 -m worker-python.app.worker; sleep 5; done) & exec java -Xms96m -Xmx256m -Xss512k -XX:MaxMetaspaceSize=128m -XX:ReservedCodeCacheSize=32m -XX:+UseSerialGC -XX:ActiveProcessorCount=1 -jar /app/app.jar"]
