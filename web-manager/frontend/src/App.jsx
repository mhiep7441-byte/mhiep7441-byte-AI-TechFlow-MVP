...
function TaskCard({ task, actions }) {
  ...
  return <article><div className="card-top"><span className={`priority ${task.priority}`}>{task.priority}</span><button className="icon" onClick={remove} aria-label="Xóa"><Trash2 /></button></div><h3>{task.title}</h3>{task.description && <p>{task.description}</p>}{task.topic && <div className="topic"><Sparkles />{task.topic}</div>}{task.errorMessage && <div className="error">{task.errorMessage}</div>}<footer><span>{task.dueDate ? <><CalendarDays />{task.dueDate}</> : 'Chưa có hạn'}</span><select value={task.status} disabled={task.status === 'GENERATING'} onChange={(event) => actions.updateTask(task, event.target.value)}>{statuses.filter((status) => status !== 'GENERATING').map((status) => <option key={status} value={status}>{labels[status]}</option>)}</select></footer>{task.topic && task.status !== 'GENERATING' && <button className="generate" onClick={() => actions.generateDraft(task.id)}><CirclePlay />Tạo video nháp</button>}{task.outputPath && <small className="path">{task.outputPath}</small>}</article>;
}
